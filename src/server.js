...
app.post('/postcall-coaching', async (req, res) => {
  try {
    ...
    const userMessageParts = [
      ...
      '',
      'Your tasks (these map directly to the UI sections):',
      '- For "Goal & Outcome": use goalSummary, goalAchieved, and goalComment to explain briefly what the goal of the call was and how far Zax moved towards it.',
      '- For "Your Next Move" (internal actions): use primaryNextAction and nextActionSteps. Write these as clear action items for Zax and his internal team only (no client-facing language here).',
      '- For "Client Follow-Up": use followUpsForClient. Write this as what the client should receive next (emails, documents, meetings, promises). This text can be reused directly in client communication, so avoid internal coaching language here.',
      '- For "Risk & Opportunity": use riskLevel and opportunitySize. Use simple buckets only: riskLevel must be one of "low", "medium", or "high"; opportunitySize must be one of "small", "medium", or "large". Include the reasoning inside goalComment or coachingTips, not as extra fields.',
      '- For "Coaching": use strengths, improvementAreas, missedQuestions, and coachingTips. These are internal coaching points for Zax about his discovery skills and behaviour on this call.',
      '',
      'General rules:',
      ...
      '- Do NOT repeat long call summaries or restate the transcript; keep each item concise and action-focused. One or two sentences per bullet is enough.',
...
app.post('/jobs/:jobId/send-detailed-report', async (req, res) => {
  const { jobId } = req.params || {};
  ...
  try {
    ...
    const rawAnalysis = jobRow.analysisJson;
    if (!rawAnalysis || typeof rawAnalysis !== 'string' || !rawAnalysis.trim()) {
      return res.status(400).json({ error: 'No analysis available for this job' });
    }
    ...
    const combineLines = (...values) => {
      const lines = [];
      for (const val of values) {
        lines.push(...normalizeLines(val));
      }
      return lines;
    };

    const clientName =
      (typeof analysis?.CLIENT_NAME === 'string' && analysis.CLIENT_NAME.trim()) ||
      (typeof analysis?.clientName === 'string' && analysis.clientName.trim()) ||
      '';
    const company =
      (typeof analysis?.CLIENT_INDUSTRY === 'string' && analysis.CLIENT_INDUSTRY.trim()) ||
      (typeof analysis?.company === 'string' && analysis.company.trim()) ||
      '';
    const summary =
      (typeof analysis?.callSummary === 'string' && analysis.callSummary.trim()) ||
      (typeof analysis?.overallSummary === 'string' && analysis.overallSummary.trim()) ||
      (typeof jobRow.resultSummary === 'string' && jobRow.resultSummary.trim()) ||
      '';

    const overviewLines = [];
    if (clientName) overviewLines.push(`Client: ${clientName}${company ? ` (${company})` : ''}`);
    if (summary) overviewLines.push(`Summary: ${summary}`);
    if (jobRow.createdAt) overviewLines.push(`Call date/time: ${jobRow.createdAt}`);

    const painPoints = combineLines(analysis?.painPoints, analysis?.RED_FLAGS);
    const automations = combineLines(
      analysis?.AUTOMATIONS_LIST,
      analysis?.recommendedAutomations,
      analysis?.REVENUE_GROWTH,
      analysis?.COSTS_RESOURCES
    );
    const risks = combineLines(
      analysis?.RISK_QUALITY,
      analysis?.READINESS_CONSTRAINTS,
      analysis?.COMPETITION_CAPACITY
    );
    const nextSteps = combineLines(
      analysis?.NEXT_STEPS,
      analysis?.followUpsForClient,
      analysis?.primaryNextAction,
      analysis?.nextActionSteps
    );

    const sections = [];

    if (overviewLines.length > 0) {
      sections.push({ title: 'Meeting overview', lines: overviewLines });
    }

    const goals = combineLines(
      analysis?.TOP_PRIORITY,
      analysis?.CLIENT_OVERVIEW,
      analysis?.PLAN_LIST
    );
    if (goals.length > 0) {
      sections.push({ title: 'Client goals & context', lines: goals });
    }

    if (painPoints.length > 0) {
      sections.push({ title: 'Key pain points', lines: painPoints });
    }

    if (automations.length > 0) {
      sections.push({
        title: 'Recommended automations / improvements',
        lines: automations,
      });
    }

    if (risks.length > 0) {
      sections.push({ title: 'Risks / deal outlook', lines: risks });
    }

    if (nextSteps.length > 0) {
      sections.push({ title: 'Next steps', lines: nextSteps });
    }

    const sectionToHtml = (title, lines) => {
      if (!lines || lines.length === 0) return '';
      const items = lines.map((line) => `<li>${htmlEscape(line)}</li>`).join('');
      return `<h2>${htmlEscape(title)}</h2><ul>${items}</ul>`;
    };

    const htmlBody = [
      '<h1>Post-call report</h1>',
      ...sections.map((section) => sectionToHtml(section.title, section.lines)),
    ].join('');

    let textBody = 'Post-call report';
    ...
    await sendDetailedReportEmail({
      to: 'future@kalyanai.io',
      subject: `Kalyan AI – Post-call report for job ${jobId}`,
      text: textBody,
      html: htmlBody,
    });

    console.log('Detailed post-call report sent for job', jobId);
    return res.json({ success: true });
  } catch (error) {
    console.error('Failed to send detailed report', error);
    return res.status(500).json({ error: 'Failed to send detailed report' });
  }
});
